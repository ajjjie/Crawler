...
for country, char_infos in all_char_infos.items():
    ...
    with open(path, 'w', encoding='utf-8') as f:
        ...
        for place, info in place_infos.items():
            if place==country:
                continue
            f.write(f'### {country}地点{place}介绍\n{info}\n')
        f.write(f'\n## {country}角色介绍，共有{len(char_infos)}个角色\n')
        for i, info in enumerate(char_infos):
            f.write(f'### {country}第{i+1}个角色：{info.name}\n')
            f.write(f'#### {info.name}基本信息\n')
            for i in range(len(info.info)):
                f.write(f'{info.info.index[i]}:{info.info[i]}\n')
            f.write(f'#### {info.name}攻略\n')
            for i in range(len(info.rec)):
                if i==0:
                    f.write(f'##### {info.name}圣遗物\n')
                elif i==6:
                    f.write(f'##### {info.name}武器\n')
                f.write(f'{info.rec.index[i]}:{info.rec[i]}\n')
            f.write(f'#### {info.name}背景故事：\n')
            f.write(info.bg + '\n')